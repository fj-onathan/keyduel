...
export function ProfileAchievements({items}: Props) {
  return (
    <article className="profile-panel profile-main-card self-start">
      ...
      {items.length === 0 ?
        <p className="profile-panel-subtitle text-xs sm:text-sm">No monthly top 10 badges yet.</p> : null}
      {items.length > 0 ? (
        <ul className="mt-2 sm:mt-3 grid gap-1.5 sm:gap-2 list-none pl-0">
          {items.map((item) => (
            ...
            >
              <span
                className={`profile-achievement-badge badge-${item.badgeType} text-[0.58rem] sm:text-[0.64rem]`}>{badgeLabel(item.badgeType)}</span>
              <strong
                className="min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-[#e7d3be]">{item.hubTitle}</strong>
